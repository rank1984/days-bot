...
def update_all_trades():
    if not is_market_open():
        print("[TradeMonitor] Market closed.")
        return
    ...
    for trade in trades:
        ...
        try:
            ...
            if exit_reason:
                update_trade_monitor(ticker,
                    current_price=exit_price, high=high, low=low,
                    mfe=mfe, mae=mae,
                    tp1_hit=(exit_reason=="TP1"), tp2_hit=(exit_reason=="TP2"), stop_hit=(exit_reason=="STOP"),
                    exit_price=exit_price, pnl=pnl, win=win
                )
                print(f"[TradeMonitor] ✅ {ticker} closed: {exit_reason} | PnL: {pnl:.2f}%")
            else:
                update_trade_monitor(ticker,
                    current_price=last_price, high=high, low=low,
                    mfe=mfe, mae=mae
                )
